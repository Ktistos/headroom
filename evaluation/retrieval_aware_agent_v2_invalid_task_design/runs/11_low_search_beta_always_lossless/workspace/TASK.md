...
Copy the complete zero-based record at position **1** into `EXPECTED_RECORD` in `solution.py`. Preserve every key and value. Then run `python -m unittest -q`. The visible test checks shape; a hidden grader checks the complete value.
